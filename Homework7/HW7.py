...
def calc_hermite_coefficient(pt0, pt1, slope):
    """
    구간별 함수 g(x)에 사용될 3차항의 계수 계산
    slope는 pt0과 pt1에서의 기울기
    """
    x0, y0 = pt0
    x1, y1 = pt1
    s0, s1 = slope
...
def draw_cubic_hermite_curve(points, color=BLUE, steps=100, thick=3):
    """
    Piecewise cubic hermite interpolation 을 이용한 곡선 생성
    """
...
def b(k, i, t):
    def __func(x):
        ...
        if t[i+k] == t[i]:
            ...
            c1 = (x-t[i])/(t[i+k] - t[i]) * b(k-1, i, t)(x)
        if t[i+k+1] == t[i+1]:
            c2 = 0.0
        else:
            c2 = (t[i+k+1] - x) / (t[i+k+1] - t[i+1]) * b(k-1, i+1, t)(x)
        return c1 + c2
    return __func
...
def b_spline(t, c, k):
    def __func(x):
        n = len(t) - k - 1
        B = np.array([b(k, i, t)(x) for i in range(n)])
        print(c.T.shape, B.shape)
        res = c.T @ B
        return res
    return __func


def draw_b_spline(points, degree=3, color=BLUE, steps=100, thick=3):
    mat_points = np.array(points)
    cnt = len(mat_points)

    degree = np.clip(degree, 1, cnt-1)

    knots = np.clip(np.arange(cnt+degree+1)-degree, 0, cnt-degree)

    max_p = cnt - degree
    xx = np.linspace(0, max_p, steps)

    bspline = b_spline(knots, mat_points, degree)
...
def main():
    ...
    while not done:
        ...
        if len(rm.rectangles) >= 2:
            ...
            calc_time, _ = time_it(draw_b_spline, tuple(rect.center for rect in rm.rectangles), steps=100)
